                  ...
                  <table className="w-full border-collapse border border-gray-300">
                    ...
                    <tbody>
                      {mentee.subjects.map((sub) => (
                        <tr key={sub.code} className="text-center">
                          ...
                          <td className="border px-2 py-1">{sub.credit}</td>
                          <td className={`border px-2 py-1 ${sub.status === "Pass" ? "text-green-600" : "text-red-600"}`}>{sub.status}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
      ...
      {chartData && selectedMentee && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-4 rounded shadow-lg max-w-lg w-full relative">
            ...
            <h2 className="text-lg font-semibold mb-2">Chart for {selectedMentee}</h2>
            <img src={chartData} alt="Student Chart" className="w-full h-auto" />
          </div>
        </div>
      )}
    </div>
