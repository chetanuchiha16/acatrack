    ...
    try {
      ...
      if (res.ok) {
        setPerformanceData({
          ...
          study_summary: data.study_summary || "Focus on overall improvement.", // use the concise summary
        });
        fetchChart();
        setOpenPerf(true);
      } else {
  ...
  return (
    <div className="p-2 rounded space-y-2">
      ...
      <div className="flex flex-col sm:flex-row gap-2">
        <button
          onClick={() => { if (!aiData) fetchAIInsights(); else setOpenAI(!openAI); }}
          disabled={loadingAI}
        ...
        >
          ✨ AI Insights {loadingAI && <span className="ml-2 animate-spin">⏳</span>}
          {!loadingAI && aiData && <span className="ml-2">{openAI ? "▲" : "▼"}</span>}
        </button>

        <button
          onClick={() => { if (!performanceData) fetchPerformance(); else setOpenPerf(!openPerf); }}
        ...
        >
          📊 Performance Dashboard {loadingPerf && <span className="ml-2 animate-spin">⏳</span>}
          {!loadingPerf && performanceData && <span className="ml-2">{openPerf ? "▲" : "▼"}</span>}
        </button>
      </div>
      ...
      {openPerf && performanceData && (
        <div className="mt-2 space-y-4 bg-gray-50 p-4 rounded-lg shadow-inner text-gray-800">
          <h3 className="font-semibold text-blue-600 mb-2">📊 Student Performance Dashboard</h3>

          {/* Summary */}
          <div className="grid grid-cols-2 gap-2 text-sm mb-2">
            <div className="bg-gray-100 px-2 py-1 rounded"><b>Name:</b> {performanceData.name}</div>
            <div className="bg-gray-100 px-2 py-1 rounded"><b>USN:</b> {performanceData.usn}</div>
            <div className="bg-gray-100 px-2 py-1 rounded"><b>Semester:</b> {semester}</div>
            <div className="bg-gray-100 px-2 py-1 rounded"><b>Total Marks:</b> {performanceData.total_marks}</div>
            <div className="bg-gray-100 px-2 py-1 rounded"><b>Percentage:</b> {performanceData.percentage.toFixed(2)}%</div>
            <div className="bg-gray-100 px-2 py-1 rounded"><b>SGPA:</b> {performanceData.sgpa.toFixed(2)}</div>
            <div className="bg-gray-100 px-2 py-1 rounded"><b>CGPA:</b> {performanceData.cgpa.toFixed(2)}</div>

            {performanceData.predicted_next_sgpa && (
              <div className="bg-gray-100 px-2 py-1 rounded">
                <b>Predicted Next SGPA:</b> {performanceData.predicted_next_sgpa.toFixed(2)}
              </div>
            )}
          </div>
            ...
            <table className="w-full border text-center">
              <thead>
                <tr className="border bg-gray-100">
                  ...
                  <th className="border px-2 py-1">Advice / Tips</th>
                </tr>
              </thead>
              <tbody>
                {performanceData.subject_analysis.map((sub) => (
                  <tr key={sub.code} className={`border ${getSubjectColor(sub)}`}>
                    <td className="border px-2 py-1">{sub.code}</td>
                    <td className="border px-2 py-1">{sub.subject_name}</td>
                    <td className="border px-2 py-1">{sub.ia}</td>
                    <td className="border px-2 py-1">{sub.see}</td>
                    <td className="border px-2 py-1">{sub.total}</td>
                    <td className="border px-2 py-1">{sub.status}</td>
                    <td className="border px-2 py-1 text-left text-sm">
                      {sub.advice && <p className="text-red-600 font-medium">{sub.advice}</p>}
                      {sub.tips && <p className="text-green-600">{sub.tips}</p>}
                    </td>
                  </tr>
                ))}
              </tbody>
          ...
          <div>
            <h4 className="font-semibold mb-2">Overall Improvement Advice</h4>
          ...
          {performanceData.study_summary && (
            <div className="bg-yellow-50 p-4 rounded-lg shadow-inner text-gray-800 mt-4">
              <h4 className="font-semibold text-yellow-700 mb-2">📌 Overall Study Advice</h4>
              <p className="text-sm">{performanceData.study_summary}</p>
            </div>
          )}
          ...
          {chartUrl && (
            <div className="mt-2">
              <h4 className="font-semibold mb-2">Subject Marks Chart</h4>
              <img src={chartUrl} alt="Student Marks Chart" className="w-full" />
            </div>
          )}
        </div>
      )}
