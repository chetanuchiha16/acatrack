...
def get_student_history_usn(usn, semesters):
    ...
    for sem in semesters:
        try:
            ...
            s = Student(usn=usn, semester=sem, batch_year=batch_year)
            s.calculate_sgpa()
...
def ai_summary():
    ...
    for sem in semesters_list:
        try:
            ...
            student_data["semesters"][sem] = {
                ...
                "credits": safe_list(getattr(s, "obtained_credits", 0) or getattr(s, "credits", 0)),
    ...
    summary = {
        ...
        "backlog_status": (
            ...
            if total_backlog_credits > 0 else
            "✅ No backlogs — academic record is clear."
        )
    }
