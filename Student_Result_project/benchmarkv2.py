...
def run_single_user_baseline(base_dir):
    print("📋 Running Single User Baseline (1 VU, 10 Iterations with P95 RAM tracking)...")
    ...
    for route in routes:
        ...
        try:
            ...
            subprocess.run(
                ["k6", "run", "load_testv2.js", "--vus", "1", "--iterations", "10", 
            ...
            avg_impact = max(0, (sum(route_ram_samples)/len(route_ram_samples)) - ram_baseline) if route_ram_samples else 0
            ...
            with open(baseline_summary_path, "r") as f:
                ...
                if b_metrics:
                    ...
                    if not metric: metric = b_metrics[0]
...
"""
    ...
    if baseline_data:
        ...
        ram_sorted = sorted(baseline_data, key=lambda x: x["p95_ram_impact"], reverse=True)
        md_content += f"""
