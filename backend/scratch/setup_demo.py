...
async def main():
    ...
    async with AsyncSessionLocal() as session:
        ...
        if not section:
            ...
            print(f"ℹ️ Section {section_name} for Batch {batch_year} already exists (ID: {section.id})")
        ...
        if not teacher:
            teacher = Teacher(username=teacher_username, name="Demo Staff", email="demostaff@example.com")
            session.add(teacher)
            await session.commit()
        ...
        if not subject:
            subject = Subject(subject_code=subject_code, subject_name="Introduction to Computer Science", semester="sem1", credits=4)
            session.add(subject)
            await session.commit()
            await session.refresh(subject)
            print(f"✅ Created Subject: {subject.subject_name} ({subject.subject_code})")
        else:
            print(f"ℹ️ Subject already exists: {subject.subject_name} ({subject.subject_code})")
        ...
        if not student:
            ...
            session.add(student)
            await session.commit()
            print(f"✅ Enrolled Student: {student.name} ({student.usn}) in Section {section_name}")
        else:
        ...
        if not assignment:
            ...
            await session.commit()
            print(f"✅ Created SubjectAssignment: {teacher_username} -> {subject_code} in Section {section_name}")
        else:
            print(f"ℹ️ SubjectAssignment already exists for {teacher_username} in Section {section_name}")
            
        # 6. Refresh Batch Lifecycle Status & Counts
        await BatchLifecycleService.refresh_counts_and_status(session, batch_year)
        print("✅ Refreshed Batch Lifecycle status and assignment/student counts successfully!")
