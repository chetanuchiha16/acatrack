...
async def main():
    ...
    async with AsyncSessionLocal() as session:
        ...
        if not students:
            print(
                "❌ No demo students found in the database. Please run setup_demo.py first."
            )
            return
        ...
        for student in students:
            ...
            stmt_res = select(AcademicResult).where(
                AcademicResult.student_id == student.id
            )
